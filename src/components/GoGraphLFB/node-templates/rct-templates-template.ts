...
export function createRCTNodeTemplate(clickHandler?: ((e: go.InputEvent, thisObj: go.GraphObject) => void) | null): go.Node {
	...
	console.log('findLinksConnected ======>>>', node.findLinksConnected().count);
	console.log('findLinksOutOf ======>>>', node.findLinksOutOf().count);
	console.log('findLinksInto ======>>>', node.findLinksInto().count);
