	...
	incremental: function (FeAdd, FeDel, F, R) {
		var Rdel = [],
			...
			overDeletionEvaluationLoop = function () {
				...
				Solver.evaluateRuleSet(Rdel, Utils.uniques(Utils.uniques(Fi, Fe), FeDel)).then(
					function (values) {
			...
			rederivationEvaluationLoop = function () {
				...
				Solver.evaluateRuleSet(Rred, Utils.uniques(Fe, Fi)).then(
					function (values) {
			...
			insertionEvaluationLoop = function () {
				...
				Solver.evaluateRuleSet(Rins, superSet).then(
					function (values) {
						FiAddNew = values.cons;
	...
	tagging: function (FeAdd, FeDel, F, R) {
		var newExplicitFacts,
			...
			evaluationLoop = function () {
				...
				Solver.evaluateRuleSet(Rins, F, true).then(function (values) {
					FiAdd = values.cons;
