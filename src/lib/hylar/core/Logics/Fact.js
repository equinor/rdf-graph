...
Fact.prototype = {
	...
	truncatedString: function () {
		...
		if (this.falseFact) {
			...
			spo =
				'(' +
				Utils.removeBeforeSharp(this.subject) +
				', ' +
				Utils.removeBeforeSharp(this.predicate) +
				', ' +
				Utils.removeBeforeSharp(this.object) +
				')';
		}
	...
	equivalentTo: function (fact) {
		if (this.explicit != fact.explicit || this.subject != fact.subject || this.predicate != fact.predicate || this.object != fact.object) {
			return false;
		}
		return true;
	},
