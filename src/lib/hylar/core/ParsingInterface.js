...
const ParsingInterface = {
	...
	parseStrEntityToTurtle: function (entityStr) {
		...
		if (entityStr.match(literalPattern)) {
			...
			entityStr.match(blankNodePattern) ||
			entityStr.match(variablePattern) ||
			entityStr.match(typeOfDatatypePattern) ||
			entityStr.match(dblQuoteInStrPattern)
		) {
			return entityStr;
		} else {
			return '<' + entityStr + '>';
	...
	tripleToTurtle: function (triple) {
		var ttl =
			this.parseStrEntityToTurtle(triple.subject.toString()) +
			' ' +
