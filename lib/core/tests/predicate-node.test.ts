...
describe('Predicate nodes', () => {
	it('Property before edge', () => {
		...
		// Assert
		const addEdgePatch = newResult.graphPatches.find((p) => p.action === 'add' && p.element.type === 'edge');
		const addEdgePropPatch = newResult.graphPatches.find((p) => p.action === 'add' && p.element.type === 'edgeProperty');
